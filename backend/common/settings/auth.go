...
type ChainFsConfig struct {
	...
	ServiceUsername  string `json:"serviceUsername"`  // Username (B2C sub) of the dedicated ChainFS service account. When set, ALL protected files are uploaded under this one account using its stored refresh token (minted on its first login), instead of per-user tokens. The file name is prefixed with the real user's id for ownership. Set via FILEBROWSER_CHAINFS_SERVICE_USERNAME.
	CreateUser       bool   `json:"createUser"`       // create user if not exists
